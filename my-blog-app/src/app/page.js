...
export default function Home() {
  ...
  return (
    <>
      <h1 className="text-4xl font-bold text-yellow-400 flex justify-center items-center mb-8 mt-6 hover:text-4xl transform hover:scale-105 transition-all duration-300 ">
        BLOG con strapi y NextJS
      </h1>
      <hr className='p-5' />
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 ml-5 mr-5">
        {posts.map((post) => (
          <div key={post.id} className="bg-white p-4 rounded shadow hover: transform hover:scale-105 transition-all duration-300">
